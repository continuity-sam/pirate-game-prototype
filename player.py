...
class Player(pygame.sprite.Sprite):
	def __init__(self, pos, groups, collision_sprites, semi_collision_sprites):
		...
		self.image = pygame.image.load(join(base_dir, 'data', 'graphics', 'player', 'idle', '0.png'))
	...
	def move(self, dt):
		...
		if not self.on_surface['floor'] and any((self.on_surface['left'], self.on_surface['right'])) and not self.timers['wall slide block'].active:
			...
			self.direction.y += self.gravity / 2 * dt
			self.collision('vertical') # not in video but helps it jump (glitch week 2  - week 6)
...
# 5games-main\pirate_game\data\graphics\player\idle\0.png